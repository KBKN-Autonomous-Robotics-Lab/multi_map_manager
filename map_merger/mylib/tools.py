...
class Tools(tk.Frame):
    def __init__(self, master, theme, **kwargs):
        ...
        ## move ボタン
        icon = Image.open(
            Path(__file__).parent.parent / Path("icon", "move_button.png")
        )
        icon = icon.resize((30, 30))
        self.move_btn_icon = ImageTk.PhotoImage(image=icon)
        self.move_btn = tk.Label(
            self.btn_frame, image=self.move_btn_icon, bg=theme["main"]
        )
        self.move_btn.bind(
            "<Enter>", lambda event, btn=self.move_btn: self.btn_entry(event, btn)
        )
        self.move_btn.bind(
            "<Leave>", lambda event, btn=self.move_btn: self.btn_leave(event, btn)
        )
        self.move_btn.bind(
            "<Button-1>",
            lambda event, btn=self.move_btn, mode="move": self.btn_clicked(
                event, btn, mode
            ),
        )
        ...
        ## rotate ボタン
        icon = Image.open(
            Path(__file__).parent.parent / Path("icon", "rotate_button.png")
        )
        icon = icon.resize((30, 30))
        self.rot_btn_icon = ImageTk.PhotoImage(image=icon)
        self.rot_btn = tk.Label(
            self.btn_frame, image=self.rot_btn_icon, bg=theme["main"]
        )
        self.rot_btn.bind(
            "<Enter>", lambda event, btn=self.rot_btn: self.btn_entry(event, btn)
        )
        self.rot_btn.bind(
            "<Leave>", lambda event, btn=self.rot_btn: self.btn_leave(event, btn)
        )
        self.rot_btn.bind(
            "<Button-1>",
            lambda event, btn=self.rot_btn, mode="rotate": self.btn_clicked(
                event, btn, mode
            ),
        )
        ...
        ## lock, unlock ボタン
        icon = Image.open(
            Path(__file__).parent.parent / Path("icon", "locked_button.png")
        )
        icon = icon.resize((30, 30))
        self.locked_icon = ImageTk.PhotoImage(image=icon)
        icon = Image.open(
            Path(__file__).parent.parent / Path("icon", "unlocked_button.png")
        )
        icon = icon.resize((30, 30))
        self.unlocked_icon = ImageTk.PhotoImage(image=icon)
        self.lock_btn = tk.Label(
            self.btn_frame, image=self.unlocked_icon, bg=theme["main"]
        )
        self.lock_btn.bind(
            "<Enter>", lambda event, btn=self.lock_btn: self.btn_entry(event, btn)
        )
        self.lock_btn.bind(
            "<Leave>", lambda event, btn=self.lock_btn: self.btn_leave(event, btn)
        )
    ...
    def btn_clicked(self, event, btn: tk.Label, mode):
        if (len(self.label_list) == 0) or (self.selected_layer == self.base_map_layer):
            return
        ...
        if self.map_disp.mode == self.map_disp.MoveSelected:
            self.move_btn.configure(bg=self.theme["main"])
        elif self.map_disp.mode == self.map_disp.RotateSelected:
            self.rot_btn.configure(bg=self.theme["main"])
        # MapDisplayにモードを設定
        if mode == "move":
            self.map_disp.mode = self.map_disp.MoveSelected
        elif mode == "rotate":
            self.map_disp.mode = self.map_disp.RotateSelected
    ...
    def set_multimaps(self, dirpath: Path, wp_path: Path):
        ...
        while True:
            ...
            if map_idx == 0:
                ...
                self.tra_bar.configure(variable=self.selected_layer.map_transparency)
            if "change_map" in point["point"].keys():
                map_idx = int(point["point"]["change_map"])
            wp_idx += 1
    ...
    def append_layer(self, path: Path, base=None):
        ...
        label.bind(
            "<Button-1>",
            lambda event, label=label: self.layerlabel_clicked(event, label),
        )
        if self.label_list:
            label.pack(
                expand=False,
        ...
        self.tra_bar.configure(
            variable=self.selected_layer.map_transparency, state=tk.NORMAL
        )
        self.lock_btn.configure(image=self.unlocked_icon, bg=self.theme["main"])
        self.label_list.append(label)
        if base:
            self.base_map_layer = label
        return
    ...
    def get_waypoints_yaml(self):
        ...
        waypoints = WaypointList(
            self.map_disp.waypoints_dict[self.map_disp.base_map_key].waypoints_yaml
        )
        # ベースのfinish poseをウェイポイントに変換
        finish_pose: FinishPose = self.map_disp.waypoints_dict[
            self.map_disp.base_map_key + "fp"
        ]
        # 個別のウェイポイントをリスト化
        map_idx = 1
        ...
        for label in self.label_list[1:]:
            ...
            mat = [
                [math.cos(theta), -math.sin(theta), real_x],
                [math.sin(theta), math.cos(theta), real_y],
                [0, 0, 1],
            ]
            wp_list: WaypointList = self.map_disp.waypoints_dict[
                self.map_disp.path2key(label.map_path)
            ]
    ...
    def get_merged_map(self):
        ...
        img_size = (
            int(round(lower - upper) + offset),
            int(round(right - left) + offset),
        )  # (y, x)
        ...
        free_area[y : y + h, x : x + w] = free_area[y : y + h, x : x + w] | np.where(
            img < 230, 0, 255
        )
        ...
        for i in range(1, len(self.label_list)):
            ...
            img = mymap.original_img_pil.convert("L")
            img = img.rotate(
                -math.degrees(mymap.get_rotate_angle()),
            ...
            free_area[y : y + h, x : x + w] = free_area[
                y : y + h, x : x + w
            ] | np.where(img < 230, 0, 255)
