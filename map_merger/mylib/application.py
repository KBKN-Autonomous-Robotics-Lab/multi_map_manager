...
class Application(tk.Frame):
    def __init__(self, master):
        ...
        self.open_menu.add_command(
            label="Additional map", command=self.menu_open_addtion, state="disabled"
        )
        ...
        self.file_menu.add_command(
            label="Export", command=self.menu_export, accelerator="Ctrl+E"
        )
        ...
        paned_window = tk.PanedWindow(
            self.master, sashwidth=3, bg="gray", relief=tk.RAISED
        )
        ...
        ## マップ画像を表示するフレームを追加
        paned_window.add(
            self.tools.map_disp, minsize=self.tools.map_disp.min_width, padx=2, pady=2
        )
        return
    ...
    def menu_open_base(self, event=None):
        ...
        suc = self.tools.set_base_map(
            Path(map_path).resolve(), Path(waypoints_path).resolve()
        )
        if not suc:
            return
    ...
    def menu_open_multi(self, event=None):
        ...
        if not (dirpath / Path("map0.yaml")).exists():
            return
        waypoints_path = self.open_yaml(title="Select waypoints file for the map")
        if not waypoints_path:
            return
    ...
    def menu_export(self, event=None):
        if len(self.tools.label_list) < 2:
            return
        ## サブウィンドウを作成
        win = tk.Toplevel()
        ...
        def ref_btn_callback_d(entry: tk.Entry, init_dir):
            dirpath = tkinter.filedialog.askdirectory(
                parent=win, title="Directory path to export maps", initialdir=init_dir
            )
            if not dirpath:
                return
        ...
        ref_btn["command"] = lambda entry=tbox, init_dir=init_dir: ref_btn_callback_d(
            entry, init_dir
        )
