...
class MyMap:
    def __init__(self, path: Path, map_yaml):
        ...
        self.img_origin = [
            ...
            self.original_img_pil.height + self.origin[1] / self.resolution,
        ]
        return
    ...
    def get_draw_image(self, canvas_size):
        ...
        self.pil_img = self.original_img_pil.transform(
            canvas_size,
...
class MapDisplay(tk.Frame):
    def __init__(self, master, theme, **kwargs):
        ...
        self.canvas.bind(
            "<Control-Button-1>", lambda event, scale=1.2: self.ctrl_click(event, scale)
        )
        self.canvas.bind(
            "<Control-Button-3>", lambda event, scale=0.8: self.ctrl_click(event, scale)
        )
    ...
    def plot_waypoints(self):
        for key, wp_list in self.waypoints_dict.items():
            if key[-2:] == "fp":
                ...
                img_x, img_y = self.map_dict[key[:-2]].real2image(
                    finish_pose.x, finish_pose.y
                )
                ...
                if self.waypoints_dict[key].id is not None:
                    self.canvas.delete(
                        self.waypoints_dict[key].id
                    )  # movetoだとX,毎回削除,再描画
                self.waypoints_dict[key].id = self.canvas.create_line(
                    x0,
                    y0,
                    x1,
                    y1,
            ...
            if len(wp_list.get_id_list()) == 0:  # 初めて描画する
                ...
                for id in wp_list.get_id_list():
                    ...
                    img_x, img_y = self.map_dict[key].real2image(
    ...
    def create_waypoint(self, key, waypoint: dict, mymap: MyMap):
        ...
        id = self.canvas.create_oval(
            x0, y0, x1, y1, fill="#F88", outline="#F88", tags=key + "wp"
        )
        return id
    ...
    def set_transparency(self, path: Path, alpha):
        ...
        self.canvas.itemconfigure(
            key, image=self.map_dict[key].get_draw_image((self.canv_w, self.canv_h))
        )
    ...
    def left_click_move(self, event):
        ...
        if self.mode == self.RotateSelected:
            ...
            if self.rotate_center is None:
                ...
                self.canvas.create_oval(
                    x1, y1, x2, y2, fill="#FFA", outline="#FF0", tags="rotation_center"
                )
                return
            ...
            if self.canvas.find_withtag("rotation_line"):
                ...
                self.canvas.create_line(
                    cx, cy, event.x, event.y, tags="rotation_line", width=1, fill="#FAA"
                )
            ...
            for key in self.map_dict.keys():
                ...
                self.canvas.itemconfigure(
                    key,
                    image=self.map_dict[key].get_draw_image((self.canv_w, self.canv_h)),
                )
                self.canvas.move(key + "wp", delta_x, delta_y)
                self.canvas.move(key + "fp", delta_x, delta_y)
