...
class Application(tk.Frame):
    def __init__(self, master):
        ...
        self.file_menu.add_command(
            label="Open", command=self.menu_open, accelerator="Ctrl+O"
        )
        ## File -> Save As
        self.file_menu.add_command(
            label="Save As", command=self.menu_saveas, accelerator="Ctrl+Shift+S"
        )
        ## File -> Exit メニュー
        ...
        self.edit_menu.add_command(
            label="Trim", command=self.menu_trim, accelerator="Ctrl+T"
        )
        ...
        self.canvas.bind(
            "<Control-Button-1>", lambda event, scale=1.1: self.ctrl_click(event, scale)
        )
        self.canvas.bind(
            "<Control-Button-3>", lambda event, scale=0.9: self.ctrl_click(event, scale)
        )
    ...
    def menu_open(self, event=None):
        ...
        self.origin_img = [
            -origin[0] / resolution,
            origin[1] / resolution + self.mymap.original_img_pil.height,
        ]
        self.plot_origin()
    ...
    def menu_saveas(self, event=None):
        ...
        # origin
        origin = [
            (self.trim_range[0] - self.origin_img[0]) * self.mymap.resolution,
        ...
        yaml.append(
            "occupied_thresh: " + str(self.mymap.map_yaml["occupied_thresh"]) + line
        )
    ...
    def menu_trim(self, event=None):
        ...
        if self.trimming_mode:
            self.canvas.delete(
                "upper", "right", "lower", "left", "ul", "ur", "lr", "ll"
            )
            self.trimming_mode = False
            self.set_alpha(0)
            return
        ...
        for tag, x1, y1, x2, y2 in [
            ...
            self.canvas.create_line(
                cxy[x1], cxy[y1], cxy[x2], cxy[y2], tags=tag, fill="#FFF", width=2
            )
        # 　四隅にマーカーを描画
        for tag, xidx, yidx in [("ul", 0, 1), ("ur", 2, 3), ("lr", 4, 5), ("ll", 6, 7)]:
            ...
            self.canvas.tag_bind(
                tag,
                "<B1-Motion>",
                lambda event, tag=tag: self.move_trim_range(event, tag),
            )
        return
    ...
    def set_alpha(self, a: int):
        ...
        self.canvas.itemconfigure(
            "map", image=self.mymap.get_draw_image((self.canv_w, self.canv_h))
        )
        return
    ...
    def plot_origin(self):
        ...
        if self.canvas.find_withtag("origin"):
            ...
            self.canvas.create_oval(
                x1, y1, x2, y2, tags="origin", fill="cyan", outline="blue"
            )
    ...
    def left_click_move(self, event):
        if (not self.mymap) or (self.trimming_mode):
            return
        if self.old_click_point is None:
            self.old_click_point = [event.x, event.y]
            return
